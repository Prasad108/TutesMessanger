				...
				app
						.controller(
								...
								function($scope, $http, $interval, $filter,
									...
									$scope.getExamSubjects = function(examId,discription) {
										console
												.log(examId);
										$scope.examName=discription;
										$scope.selectedExamForAddStudent.id=examId;
										//*************************http post request to get subDiv composit id of selected exam***************** 
										$http(
												{
													url : "GetSubjectDivCompIDResult/"
															+ examId,
													method : "POST",
												})
												.then(
...
<!-- ------Existing Exam----------------------------------- -->
                                  <div id="ExistingExam" class="tab-pane active" ng-show="ExistingExamList">
      								   <div class="row">
								                  <div class="col-lg-12">
								                      <section class="panel">
								                          <header class="panel-heading">
								                              Select Exam for Check Result
								                          </header>
								                          ...
								                          <div class="table-responsive">
								                            <table class="table">
								                              ...
								                              <tbody>
								                                  ...
								                                  <td>{{exam.id}}</td>
								                                  <td><a href ng-click="getExamSubjects(exam.id,exam.discription)">{{exam.discription}}</a></td>
					...
					<div ng-show="subjectNotInExam" class="alert alert-block alert-danger fade in">
				      ...
				      <strong>There is no subject selected for this exam</strong> 
                                   ...
                                   <section class="panel" ng-show="ShowSelectedSubjectTable">
				                        <div class="panel-body">
				                        <header class="panel-heading">
								                              Select For cheking perticular Divisions Result
								                          </header>	
				                             <div id="ShowSelectedSubjectTable" >
				                             <h2>Exam is      : {{examName}}</h2>
				                             <h3>Subject List :</h3>
														 ...
														 <div class="panel panel-default">
														      <div class="panel-heading">
																 <h4 class="panel-title">
																      <i style="display: none;">{{ sub.div.value }}</i><a href ng-click="OpenShow(sub,SubjectDivCompIDList)">{{ sub.inst.label }}>{{ sub.branch.label }}>{{ sub.classes.label }}>{{ sub.div.label }}></a><strong id="subStrong">{{ sub.subject.label }}</strong>
